...
midCellValue = 10
maxMinNoise = 5
delayingMovesValue = maxMinNoise * 2
...
basicUtilityFunc xo gen moves
  | toXO mb == Empty = subBoardsU + midControlU
  | toXO mb == xo    = winValue
  | otherwise        = -winValue + (movesPlayed * delayingMovesValue)
  where mb = makeMoves' moves
        movesPlayed = fromIntegral $ length moves 
        lsb i = look' mb i
        gsbU win
          | win == Empty = 0
